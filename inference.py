...
from model_concatmlp_mm_frehead_autodis_sid_all import LLM4Rec
from utils.data_utils_new import SequentialDataset, SequentialCollator
...
def train(
    ...
    prompt_template_name: str = "llama3",
    temp: float = 1e-4,
    idx: int = 0,
    num: int = 0,
):
    ...
    if task_type == 'sequential':
        ...
        user_embed, item_embed = None, pickle.load(open('/dataset/SASRec_item_embed_new.pkl', 'rb')).cuda()
        data_collator = SequentialCollator()
        text_embeds = torch.load(f'/dataset/{data_path}_llm2clip_text_emb.pt')
        pic_embeds = torch.load(f'/dataset/{data_path}_llm2clip_pic_emb.pt')

    state_dict = load_file(checkpoint_dir + '/model.safetensors', device='cpu')
    state_dict = {k: v.cuda() for k, v in state_dict.items() if 'e1' in k or 'e2' in k or 'e3' in k or 'e4' in k or 'mlp' in k or 'lora' in k or 'user_proj' in k or 'input_proj' in k or 'score' in k}
