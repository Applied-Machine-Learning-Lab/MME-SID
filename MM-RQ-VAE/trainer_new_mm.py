...
import os
#import wandb
from datasets_ours import EmbDataset
...
class Trainer(object):
    ...
    def vq_init(self):
        self.model.eval()
        original_data = EmbDataset(self.args.data_path_1,self.args.data_path_2,self.args.data_path_3)
        ...
        for batch_idx, data in enumerate(iter_data):
            data, pic, text, emb_idx = data[0], data[1], data[2], data[3]
            data, pic, text = data.to(self.device),pic.to(self.device), text.to(self.device)

            self.model.vq_initialization(data, pic, text)    
    ...
    def _train_epoch(self, train_data, epoch_idx):
        ...
        embs  = [layer.embedding.weight.cpu().detach().numpy() for layer in self.model.pic_rq.vq_layers]
        ...
        for idx, emb in enumerate(embs):
            centers, labels = self.constrained_km(emb)
            self.labels_3[str(idx)] = labels

        for batch_idx, data in enumerate(iter_data):
            data,pic,text, emb_idx = data[0], data[1], data[2], data[3]
            data, pic, text = data.to(self.device),pic.to(self.device),text.to(self.device)
            self.optimizer.zero_grad()
            out,pic_out, text_out, rq_loss,rq_loss_2,rq_loss_3, indices, indices_2, indices_3,dense_out,dense_out_2,dense_out_3 = self.model(data,pic,text, self.labels,self.labels_2,self.labels_3)

            loss,_, loss_recon, quant_loss = self.model.compute_loss(out, pic_out,text_out,rq_loss,rq_loss_2,rq_loss_3, emb_idx, dense_out,dense_out_2,dense_out_3, data, pic, text)
